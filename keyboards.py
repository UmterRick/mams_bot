...
def MyCoursesKB(name, telegram_id):
    ...
    for group in user_info[2]:
        ...
        my_group_btn = InlineKeyboardButton('▶️ Група : ' + group_text, callback_data=str(group) + ';my_group')
...
def StudentsKB(group_id):
    ...
    for student in students:
        ...
        delete_user_btn = InlineKeyboardButton('❌', callback_data=group_id + ';T_ID:' + str(student[0]) + '<delete')
...
async def CoursesKB(call, category, state, clicked_course):
    ...
    for cur_course in course:
        ...
        course_body = f'✅✅✅\nНазва курсу: {cur_course[1]}\nТренер:{cur_course[2]}\nОпис курсу: Скоро буде\n'
        clicked_course[course_id] = course_body
        inline_btn = InlineKeyboardButton(f"⚪️Перелік груп️⚪️", callback_data=course_id)
        url_btn = InlineKeyboardButton(text='📄Повний опис курсу📄', url=cur_course[3])
...
async def GroupsKB(cur_groups, user_id, group_to, state):
    ...
    for i in range(len(cur_groups)):
        ...
        if user_type == 'admin':
            delete_btn = InlineKeyboardButton("❌", callback_data=str(cur_groups[i][0]) + ';del')
            edit_btn = InlineKeyboardButton("✏️", callback_data=str(cur_groups[i][0]) + ';edit')
            students_btn = InlineKeyboardButton("👨‍ 👩‍", callback_data=str(cur_groups[i][0]) + ';students')
            keyboard.row(delete_btn, edit_btn)
            keyboard.row(students_btn)
        else:
            enroll_btn = InlineKeyboardButton("Подати заявку у групу️", callback_data=str(cur_groups[i][0]) + ';enroll')
            keyboard.row(enroll_btn)
    if user_type == 'admin':
        add_btn = InlineKeyboardButton('Додати нову групу', callback_data='add_group')
...
def NotesKB(group_id):
    ...
    for note in notifications:
        if note[1] == group_id:
            ...
            del_note_btn = InlineKeyboardButton('❌', callback_data=str(note[0]) + ';remove from:' + str(group_id))
            keyboard.row(note_btn)
            keyboard.row(del_note_btn)
    add_note_btn = InlineKeyboardButton('Додати нагадування', callback_data=str(group_id) + ';add_note')
    keyboard.row(add_note_btn)
    back_btn = InlineKeyboardButton('⬅️ Назад', callback_data=str(group_id) + ';turn_back')
    keyboard.row(back_btn)
    return keyboard
